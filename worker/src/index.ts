...
app.post(
  ...
  async (c) => {
    try {
      ...
      const { report, tokens_used, cached } = await runTriage(
        telemetryPayload,
        c.env,
      );
      ...
      return new Response(
        JSON.stringify({ cached, shareUrl, share_id: shareId, ...report }),
        {
          headers: {
...
app.get("/v1/report/:shareId", async (c) => {
  ...
  `,
    ...
    .bind(shareId)
    .first<{ report_json: string; filename: string; created_at: number }>();
...
app.post("/v1/keys", authMiddleware, async (c) => {
  ...
  `,
    ...
    .bind(crypto.randomUUID(), userId, keyHash, prefix, name)
    .run();
